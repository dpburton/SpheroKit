...
class SPKRobotV2 : SPKRobot {
    ...
    var controlService: CBService? {
        get {
            if _controlService == nil {
                _controlService = peripheral.serviceFor(SPKService.APIv2ControlService)
            }
    ...
    var commandsCharacteristic:CBCharacteristic? {
        get {
            if _commandsCharacteristic == nil {
                _commandsCharacteristic = controlService?.characteristicFor(SPKCharacteristic.apiV2Characteristic)
            }
    ...
    public func setColor(red: UInt8, green: UInt8, blue: UInt8) {
        sendCommand(packet: SetLEDColorPacketV2(red: red, green: green, blue: blue, save: true))
    }
    ...
    */
    ...
    func sendCommand(packet: CommandPacket, response: (([UInt8]) -> Void)? = nil) {
        guard let commandsCharacteristic = commandsCharacteristic else {return}
        let data = packet.dataForPacketV2()
        if response != nil {
